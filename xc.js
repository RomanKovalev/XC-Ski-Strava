...
document.getElementsByClassName('js-snw-goals-upsell media upsell')[0].style.display = 'none'
document.getElementsByClassName('js-snw-goals-upsell media upsell')[1].style.display = 'none'
document.getElementsByClassName('js-snw-goals-upsell media upsell')[2].style.display = 'none'
...
let results = fetch(
  ...
  .then((data) => {
  	if (activities.length != 0 ) {
		...
		console.log(later_activity_local, later_activity_remote);
  		if (later_activity_local == later_activity_remote) { return 1; };
	}
  	console.log('from here:', activities)
...
activities.forEach(function (element) {
  ...
  if (Date.parse(element['start_date']) >= Date.parse(prevMonday)) {
    ...
    switch (element['start_day']) {
		  case 'Mon': days_week_counter['Mon'] = days_week_counter['Mon'] + element['distance_raw'] / 1000; console.log(element['distance_raw']);
		  break
...
console.log(days_week_counter['Mon']);
